...
.env = new.env()
v = 1:length(indt)
#files = Sys.glob(paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=3/dust_emis_flx_4x5_*.RData', sep=''))
files.int = Sys.glob(paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=',frag_expt_lim,'_intmtncy=TRUE/dust_emis_flx_4x5_*.RData', sep=''))
files.noint = Sys.glob(paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=',frag_expt_lim,'_intmtncy=FALSE/dust_emis_flx_4x5_*.RData', sep=''))
F_d.noint = F_d.int = array(NaN, dim=c(length(LON.GC), length(LAT.GC), length(files.noint)*length(indt)))
...
F_d.GC.int = apply(F_d.int, c(1,2), sum, na.rm=T) * 3600 * dt 
sum(F_d.GC.int*area.GC, na.rm=TRUE)/1e9
F_d.GC.noint = apply(F_d.noint, c(1,2), sum, na.rm=T) * 3600 * dt 
sum(F_d.GC.noint*area.GC, na.rm=TRUE)/1e9
plot.field.log(F_d.GC.int, LON.GC, LAT.GC, legend.mar=5, col=WBGYR, type='def', zlim=c(-4,0))
plot.field.log(F_d.GC.noint, LON.GC, LAT.GC, legend.mar=5, col=WBGYR, type='def', zlim=c(-4,0))
plot.field.log(F_d.GC.int, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(-4,0))
plot.field.log(F_d.GC.noint, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(-4,0))
...
F_d.M2.4x5 = sp.dissolve(F_d.M2, lon.M2, lat.M2, LON.GC, LAT.GC)
plot.field.log(F_d.M2.4x5, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(-4,1))
sum(F_d.M2.4x5*area.GC, na.rm=TRUE)/1e9

R = F_d.M2.4x5/F_d.GC; R[which(is.infinite(R) | R>(1e3) | R<(1e-3))]=NaN
D = F_d.M2.4x5-F_d.GC
plot.field.log(R, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(-3,3), col=TEMP_DIFF_65)
plot.field(D, LON.GC, LAT.GC, legend.mar=5, type='sign', zlim=c(-1,1), col=TEMP_DIFF_65)

...
# dust emission code version 1: 2006 MERRA2 4x5 data for dust emissions
...
LON.GC = seq(-180,175,by=5)[indi]; LAT.GC = seq(-89,89,by=4)[indj]
...
load('GLCNMO_frc_area_4x5.RData',envir=.env); ls(.env)
frc.r.GC = .env$frc.r.GC[indi,indj]; frc.veg.GC = .env$frc.veg.GC[indi,indj]
frc.mix.GC = .env$frc.mix.GC[indi,indj]
#A_r = frc.r.GC / (frc.r.GC+frc.veg.GC+frc.mix.GC)
#A_veg = frc.veg.GC / (frc.r.GC+frc.veg.GC+frc.mix.GC)
#A_mix = frc.mix.GC / (frc.r.GC+frc.veg.GC+frc.mix.GC)
#rm('frc.r.GC','frc.veg.GC','frc.mix.GC')
#plot.field(frc.mix.GC, LON.GC, LAT.GC, col=WBGYR)
# load landfilt 
load('landfilt_4x5.RData',envir=.env)
landfilt.GC = .env$landfilt.GC[indi,indj]
# get Prigent 2005 roughness length and rock drag partition factor
load('Pr05_z0.min_4x5.RData',envir=.env)
z0.r = .env$z0.GC[indi,indj]/100   # in cm, divide by 100 to convert to m
...
f_eff.r = f_eff.r*landfilt.GC   # take away values in Caspian sea
#plot.field(f_eff.r, LON.GC, LAT.GC)
# image(f_eff.r)
# get porosity  (issue: MERRA2 porosity may be different from CESM's effective porosity)
load('MERRA2_porosity_4x5.RData', envir=.env)
poros = .env$poros.GC[indi,indj] # porosity (m3 / m3, or volumetric soil water) 
#image(poros)
#plot.field(poros, LON.GC, LAT.GC)
# get bulk density of dry soil material [kg/m^3]
bulk_den = (1 - poros)*dns_slt 
#plot.field(bulk_den, LON.GC, LAT.GC, legend.mar=4)
# use CESM's FAO clay fraction
for (aa in 1) {
  if (clay_dataset == 'FAO') {
    load('f_clay_FAO_4x5.RData',envir=.env)  # CESM's FAO (2012) clay
    f_clay.FAO.4x5 = .env$f_clay.FAO.GC  # 0-1, fraction of #plot.field(f_clay.FAO.05x06, LON.CESM, LAT.CESM, type='def', zlim=c(0,0.65))
    #plot.field(f_clay.FAO.4x5, LON.GC, LAT.GC)
    f_clay.FAO.4x5 = f_clay.FAO.4x5[indi,indj]
    plot.field(f_clay.FAO.4x5, LON.GC, LAT.GC)
    # threshold gravimetric moisture and clay limitation on dust emission
    gwc_thr = 0.01*(0.17*f_clay.FAO.4x5*100 + 0.0014*(f_clay.FAO.4x5*100)^2)*landfilt.GC
    #plot.field(gwc_thr, LON.GC, LAT.GC)
    mss_frc_cly_vld = f_clay.FAO.4x5 * landfilt.GC
    mss_frc_cly_vld[which(f_clay.FAO.4x5>0.2)] = 0.2
    #plot.field(mss_frc_cly_vld, LON.GC, LAT.GC)
    print('used FAO clay data')
  }  else if (clay_dataset == 'SG') {
    load('f_clay_SoilGrids_4x5.RData',envir=.env)
    f_clay.SG.4x5 = .env$f_clay.SG.GC
    f_clay.SG.4x5 = f_clay.SG.4x5[indi,indj]
    #plot.field(f_clay.SG.4x5, LON.GC, LAT.GC, type='def', zlim=c(0,0.65))
    # threshold gravimetric moisture and clay limitation on dust emission
    gwc_thr = 0.01*(0.17*f_clay.SG.4x5*100 + 0.0014*(f_clay.SG.4x5*100)^2)
    #plot.field(gwc_thr, LON.GC, LAT.GC)
    mss_frc_cly_vld = f_clay.SG.4x5
...
for (dd in 1:length(date_vec)) {
  date_vec[dd]
  files = c(files, Sys.glob(paste('/Volumes/SEAGATE/MERRA2/upscale_2006/REGRIDDED_MERRA2/',avg.method,'/merra2_regridded_met_4x5_', date_vec[dd], '.RData', sep='')))  
}
...
for (d in 1:length(date_vec)) {# d-th day
  ...
  USTAR = .env$USTAR.GC[i1:il,j1:jl,] # m/s
  Z0M   = .env$Z0M.GC[i1:il,j1:jl,]     # m
  RHOA  = .env$RHOA.GC[i1:il,j1:jl,]     # kg/m3
  PBLH  = .env$PBLH.GC[i1:il,j1:jl,]     # kg/m3
  #DISPH = .env$DISPH   # m
  T10M  = .env$T10M.GC[i1:il,j1:jl,]     # K
  #U10   = .env$U10.GC[i1:il,j1:jl,]  # m/s. Define U10 as the total wind speed of U10M and V10M. Don't confuse U10 with U10M. 
  #proc.time() - ptm
  SFMC  = .env$SFMC.GC[i1:il,j1:jl,]
  LAI   = .env$LAI.GC[i1:il,j1:jl,]
  SHLAND = .env$SHLAND.GC[i1:il,j1:jl,]
  #plot.field(DISPH[,,3], LON.GC, LAT.GC)
  #plot.field(RHOA[,,3], LON.GC, LAT.GC)
  #plot.field(Z0M[,,3], LON.GC, LAT.GC)
  #plot.map.log((Z0M[,,3]),LON.GC, LAT.GC,type='def',zlim=c(-4,0), legend.mar=5)
  ...
  f_bare = sweep(f_bare, c(1,2), landfilt.GC, '*')   # optional
  #f_bare_0.3 = 1 - LAI[,,3]/0.3; f_bare_0.3[which(f_bare_0.3<0)] = NaN
  #plot.field(f_bare[,,6], LON.GC, LAT.GC)
  #plot.field(f_bare_0.3, LON.GC, LAT.GC)
  #plot.field.log(LAI[,,6], LON.GC, LAT.GC, zlim=c(-2,1), legend.mar=5)
  ...
  #plot.field(SSR[,,3], LON.GC, LAT.GC, type='def', zlim=c(0.6,0.9))
  ...
  #F_eff.LC1[,,h] = (frc.r.GC*f_eff.r^3 + frc.veg.GC*SSR[,,h]^3 + frc.mix.GC*(SSR[,,h]*f_eff.r)^3)^(1/3)
  #  if (LC=='LC1') {F_eff.LC[,,h] = (A_r*f_eff.r^3 + A_veg*SSR[,,h]^3 + A_mix*(SSR[,,h]*f_eff.r)^3)^(1/3)}
  #  else if (LC=='LC0') {F_eff.LC[,,h] = (frc.r.GC*f_eff.r^3 + (frc.veg.GC+frc.mix.GC)*SSR[,,h]^3)^(1/3)}  # newly define LC0, removed mixed regime
  #}
  if (LC=='LC1') {
    f_eff_veg.comp = sweep(SSR^3, c(1,2), (frc.veg.GC), '*')
    f_eff_mix.comp = sweep(SSR^3, c(1,2), (frc.mix.GC*(f_eff.r^3)), '*')
    F_eff.LC = (sweep(f_eff_veg.comp+f_eff_mix.comp, c(1,2), (frc.r.M2*f_eff.r^3), '+')) ^ (1/3)
    rm('f_eff_veg.comp','f_eff_mix.comp')  
  } else if (LC=='LC0') {
    f_eff_veg.comp = sweep(SSR^3, c(1,2), (frc.veg.GC+frc.mix.GC), '*')
    F_eff.LC = (sweep(f_eff_veg.comp, c(1,2), (frc.r.GC*f_eff.r^3), '+')) ^ (1/3)
    rm(f_eff_veg.comp)
  }
  
  
  #plot.field(F_eff.LC[,,6], LON.GC, LAT.GC, type='def', zlim=c(0,1), col=WBGYR)
  ...
  #plot.field.log(Z0.LC1[,,6], LON.GC, LAT.GC, type='def', zlim=c(-5,0), col=tim.colors(32)[32:1], legend.mar=5)
  #plot.field.log(Z0M[,,6], LON.GC, LAT.GC, type='def', zlim=c(-5,0), col=tim.colors(32)[32:1], legend.mar=5)
  #plot.field.log(z0.r*landfilt.GC, LON.GC, LAT.GC, type='def', zlim=c(-5,0), col=tim.colors(32)[32:1], legend.mar=5)   # also in m
  ...
  #plot.field(USTAR.aero.neu[,,8] / USTAR[,,8], LON.GC, LAT.GC, type='def', zlim=c(0,2), col=TEMP_DIFF_65)
  # then also reconstruct ustar under non-neutral condition
  #plot.field(USTAR[,,6]*landfilt.GC, LON.GC, LAT.GC, type='def', zlim=c(0,0.8), col=WBGYR)
  #plot.field(USTAR.aeo.nonneu[,,6], LON.GC, LAT.GC, type='def', zlim=c(0,0.8), col=WBGYR)
  #plot.field(USTAR.aeo.nonneu[,,6]/USTAR.aeo.neu[,,6], LON.GC, LAT.GC, type='def', zlim=c(0,2), col=TEMP_DIFF_65)
  #plot.field(USTAR.aeo.neu[,,6], LON.GC, LAT.GC, type='def', zlim=c(0,0.8), col=WBGYR)
  #A = apply(USTAR, c(1,2), mean, na.rm=T)
  #plot.field(A*landfilt.GC, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(0,0.7))
  #A = apply(USTAR.aeo.nonneu, c(1,2), mean, na.rm=T)
  #plot.field(A, LON.GC, LAT.GC, legend.mar=5, type='def', zlim=c(0,0.7))
  ...
  #plot.field(ustar_ft.dry[,,6], LON.GC, LAT.GC)
  ...
  #plot.field(gwc_sfc[,,6], LON.GC, LAT.GC)
  #plot.field.log(gwc_sfc[,,6], LON.GC, LAT.GC, type='def',zlim=c(-3,0), legend.mar=5)
  ...
  #proc.time() - ptm
  #plot.field(frc_thr_wet_fct[,,6], LON.GC, LAT.GC, type='def', zlim=c(1,3.6))
  
  # then get fluid and impact thresholds (now assume using FAO's moisture effect)
  ustar_ft.wet = ustar_ft.dry * frc_thr_wet_fct
  #ustar_it = ustar_ft.dry * B_it * landfilt.GC
  ustar_it = B_it * sweep((ustar_ft.dry), c(1,2), landfilt.GC, '*')
  #plot.field(ustar_it[,,6], LON.GC, LAT.GC)
  #plot.field(ustar_ft.wet[,,6], LON.GC, LAT.GC)
  #A = apply(USTAR - ustar_it, c(1,2), mean, na.rm=T)
  #plot.field(A*landfilt.GC, LON.GC, LAT.GC, legend.mar=5, type='sign', zlim=c(-0.5,0.5), col=TEMP_DIFF_65)
  #A = apply(USTAR.aeo.nonneu - ustar_it, c(1,2), mean, na.rm=T)
  #plot.field(A, LON.GC, LAT.GC, legend.mar=5, type='sign', zlim=c(-0.5,0.5), col=TEMP_DIFF_65)
  ...
  if (use.intermittency=='TRUE') {
    ...
    u_mean_slt = (wnd_frc_slt/k) * log(0.1 / 1e-4)  # We used z0a = 1e-4 m in GC and assumed. Should we change to aeolian data here?
  ...
  #plot.field(u_mean_slt[,,6], LON.GC, LAT.GC)
  #plot.field(u_sd_slt[,,6], LON.GC, LAT.GC)
  #plot.field(u_fld_thr[,,6], LON.GC, LAT.GC)
  #plot.field(u_impct_thr[,,6], LON.GC, LAT.GC)
  #plot.field(intrmtncy_fct[,,6], LON.GC, LAT.GC)
  #A = apply(intrmtncy_fct, c(1,2), mean, na.rm=T)
  #plot.field(A, LON.GC, LAT.GC, col=WBGYR, legend.mar=5, type='def', zlim=c(0,1))
  ...
  #plot.field.log(A/C_d0, LON.GC, LAT.GC, type='def', zlim=c(-5,0), legend.mar=5)
  ...
  #plot.field(A, LON.GC, LAT.GC, legend.mar=5)
  ...
  for (aa in 1) {
    if (use.intermittency == 'TRUE') {
      ...
      #plot.field.log(F_d_sum, LON.GC, LAT.GC, legend.mar=5, col=WBGYR)
  ...
  #filename = paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=3/dust_emis_flx_4x5_', date_vec[d], '.RData', sep='')
  filename = paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=',frag_expt_lim,'_intmtncy=',use.intermittency,'/dust_emis_flx_4x5_', date_vec[d], '.RData', sep='')
  save('F_d', 'LON.GC', 'LAT.GC', file=filename)
  ...
  #filename = paste('/Volumes/SEAGATE/MERRA2/upscale_2006/OUTPUTS_',clay_dataset,'_',LC,'_globe_fragexpt=3/vars_for_upscale_4x5_', date_vec[d], '.RData', sep='')
